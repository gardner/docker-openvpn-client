...
[`ghcr.io/gardner/openvpn-client`](https://github.com/users/wfg/packages/container/package/openvpn-client) is a containerized OpenVPN client.
...
```
docker pull ghcr.io/gardner/openvpn-client
```
...
docker run --detach \
  ...
  ghcr.io/gardner/openvpn-client
```
...
services:
  openvpn-client:
    image: ghcr.io/gardner/openvpn-client
...
Once you have container running `ghcr.io/gardner/openvpn-client`, run the following command to spin up a temporary container using `openvpn-client` for networking.
